...
describe('List Include', () => {
  ...
  it('should include matching user documents with additional filter', async () => {
    const response = await request(app)
      .post('/api/orgs/_extra')
      .set('user', 'admin')
      .send({
    ...
    for (let x = 0; x < response.body.length; x++) {
      const org = response.body[x];
      expect(
        org.users.every((user) => user.orgs.some((orgId) => String(orgId) === String(org._id)) && user.role === 'user'),
      ).to.true;
    }
  });

  it('should include matching user documents with additional filter2', async () => {
    const response = await request(app)
      .post('/api/orgs/_extra')
      .set('user', 'admin')
      .send({
    ...
    for (let x = 0; x < response.body.length; x++) {
      ...
      expect(
        org.users.every(
          (user) => user.orgs.some((orgId) => String(orgId) === String(org._id)) && user.role === 'admin',
        ),
      ).to.true;
    }
  });
